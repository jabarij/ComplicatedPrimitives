...
{
    public static QuickSort<T> ForComparer<T>(IComparer<T> comparer) => new(comparer);

    public static QuickSort<T> ForComparer<T>(Func<T, T, int> comparer) => new(comparer);

    public static QuickSort<T> ForComparable<T>() where T : IComparable<T> => new((x, y) => x.CompareTo(y));

    public static QuickSort<T> ForComparable<T, TValue>(Func<T, TValue> getValue) where TValue : IComparable<TValue> =>
        new((x, y) => getValue(x).CompareTo(getValue(y)));
}