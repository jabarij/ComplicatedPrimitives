...
public readonly struct LimitValue<T> :
    IEquatable<LimitValue<T>>
    where T : IComparable<T>
{
    public static readonly LimitValue<T> Infinity = new();

    private readonly bool _isFinite;

    public LimitValue(T value, LimitType type = default)
    {
        Value = value;
        Type = type;
        _isFinite = true;
