...
partial class RangeExtensionsTests
{
    public class MergeOfGenericRanges : RangeExtensionsTests
    {
        ...
        public static IEnumerable<object?[]> GetTestData(string dataSourceName)
            => TestsBase.GetTestData(typeof(MergeOfGenericRanges), dataSourceName);
        ...
        {
            public required IEnumerable<RangeOfDecimals> Ranges { get; init; }
            public RangeOfDecimals? Expected { get; private set; }
        ...
        {
            // arrange
            var ranges = new List<RangeOfDecimals>
            {
                new(1m, 2m, rightLimit: LimitType.Open),
                new(2m, 3m, leftLimit: LimitType.Open)
            };
