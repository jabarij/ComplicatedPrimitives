...
internal class OpenGenericsBinderCustomization : ICustomization
{
    ...
    public OpenGenericsBinderCustomization(Type serviceType, Type factoryType)
    {
        ...
        if (factoryType.GenericTypeArguments.Length != serviceType.GenericTypeArguments.Length)
            throw new ArgumentException("Factory type must have same list of generic arguments as service type.",
                nameof(factoryType));
        ...
        var factoryMethodsList = factoryMethods.Take(2).ToList();
        
        if (factoryMethodsList.Count == 0)
            throw new ArgumentException("Factory method is missing in factory type.", nameof(factoryType));
        if (factoryMethodsList.HasMany())
    ...
    class OpenGenericsSpecimenBuilder : ISpecimenBuilder
    {
        ...
        private static readonly ConcurrentDictionary<Type, Delegate> FactoryMethods = new();
        ...
        {
            if (request is Type { IsGenericType: true } typeRequest
                && typeRequest.GetGenericTypeDefinition() == _serviceType)
            {
                var factoryMethod = FactoryMethods.GetOrAdd(typeRequest, CreateFactoryMethod, context);
...
internal class OpenGenericsBinderCustomization<T> : ICustomization
{
    ...
    public OpenGenericsBinderCustomization(
        Expression<Func<GenericParameterMock, ISpecimenContext, T>> factoryExpression)
    {
    ...
    public void Customize(IFixture fixture)
    {
        ...
        var lambda = Expression.Lambda<Func<ISpecimenContext, T>>(
            body,
            OpenGenericsBinderExpressionVisitor.SpecimenContextParameterExpression
        );
